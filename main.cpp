...
int sum_pre_issue_queue = 0; //pre_issue_queue队列的使用个
...
//    cout<<" index"<<im_index<<endl;
    ...
    while(pc > 0)
    {
         ...
         *
         */
        if(post_alu_queue != "")//得看看post_alu_que里面空不空
        {//如果有东西 就要去执行了
            ...
            */
            ...
            for(int j = 0; j < sum_pre_issue_queue && can_issue_count > 0; j++)
            {
                if(can_issue_pre[j])
                {
                    ...
                    pre_issue_queue[sum_pre_issue_queue-1] = ""; // can use i?
                    sum_pre_issue_queue--;
                    j--;
                }
            }
        ...
        */
        can_fetch = true;
        if(unit[0] != "" || unit[1] != "") // have waiting instruction of branch
        {
            can_fetch = false;
        } //分支等待。
